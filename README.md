...
- 群里 @机器人用户名，或发送 规则 / 使用规则：显示机器人使用规则

- 直接说 `签到`：每日签到
- 直接说 `上班` / `下班`：上下班打卡
- 直接说 `我的任务` / `今日记录`：查看个人信息
- 管理员回复成员消息后直接说 `任务 1. 任务内容 2. 任务内容`：给该成员分配多条任务
- 管理员直接说 `任务 @username 1. 任务内容 2. 任务内容`：按用户名分配多条任务
- 成员直接说 `进度 1 进度说明`：按自己的任务序号报备进度
- 成员直接说 `1完成.2完成`：按自己的任务序号批量报备完成
...
- 更推荐管理员“回复某个成员消息”后直接发送 `任务 1. 任务内容 2. 任务内容`，这样不依赖 username。
